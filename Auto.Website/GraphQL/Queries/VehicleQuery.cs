...
namespace Auto.Website.GraphQL.Queries
{
    public class VehicleQuery : ObjectGraphType
    {
        ...
        public VehicleQuery(IAutoDatabase db)
        {
            ...
            Field<ListGraphType<VehicleGraphType>>("Vehicles", "Запрос возвращающий все автомобили", resolve: GetAllVehicles);

            Field<VehicleGraphType>("Vehicle", "Запрос к конкретному автомобилю", new QueryArguments(MakeNonNullStringArgument("registration", "Номера машиы")), resolve: GetVehicle);

            Field<ListGraphType<VehicleGraphType>>("VehicleByColor", "Запрос возвращающий все машины с выбранным цветом", new QueryArguments(MakeNonNullStringArgument("color", "Имя цвета")), resolve: GetVehiclesByColor);
        }
